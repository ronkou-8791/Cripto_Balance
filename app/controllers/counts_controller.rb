class CountsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @count.save
        format.html { redirect_to @count, notice: '購入数量を入力いたしました' }
  ...
  def update
    respond_to do |format|
      if @count.update(count_params)
        format.html { redirect_to @count, notice: '購入数量を編集しました' }
  ...
  def destroy
    @count.destroy
    respond_to do |format|
      format.html { redirect_to counts_url, notice: '購入数量を削除しました' }
  ...
  private
    ...
    def count_params
      params.require(:count).permit(:amount)
    end
end
